...
export default {
  ...
  methods: {
    ...
    filterGenre() {
      // console.log('the length', this.filteredGenres.length);
      var idx = 4;
      var genreGroups = [];
      var group = [];
      this.filteredGenres = this.stations.filter(
        (station) => station.genre === this.genre
      );
      // for (let i = 0; i < this.filteredGenres.length; i++) {
      //   if (i !== idx) {
      //     // console.log('the idx in the if', idx, 'and the i is ', i);
      //     group.push(this.filteredGenres[i]);
